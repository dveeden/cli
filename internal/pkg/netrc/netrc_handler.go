...
import (
	...
	"github.com/confluentinc/cli/internal/pkg/encrypt"
...
type NetrcHandler interface {
	WriteNetrcCredentials(isCloud bool, ctxName string, username string, password, salt string) error
...
func (n *NetrcHandlerImpl) WriteNetrcCredentials(isCloud bool, ctxName, username, password, salt string) error {
	...
	encryptedPassword, err := encrypt.Encrypt(password, salt)
	if err != nil {
		return err
	}
